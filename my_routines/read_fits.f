      subroutine read_fits(filename, temp_dim, en_dim,            ! in
     1                     temp,en, points, kn_cross, indices, srf)  ! out
      implicit none
      integer temp_dim, en_dim
      integer i, iz, np, rownum, unit, out_ind, out_dim
      integer points(en_dim, temp_dim)
      double precision temp(temp_dim), en(en_dim)
      double precision kn_cross(en_dim, temp_dim)
...
c     rownum: index for initial photon energy (1-en_dim)
c     temp: temperature in K
c     en: energy in eV
c     kn_cross: Klein-Nishina cross section
c     out_dim: number of final photon energies
c     out_ind: index of the first final photon energy
c     srf: Super Redistribution Function
c
c
      !Open the fits file, read the 2nd extension and close it
      call read_HDU2(filename, temp_dim, en_dim, temp, en)
c
...
c
            rownum = rownum + 1
c
            !read the first 3 columns to allocate the array for the energies
            call read_HDU3_col(unit, rownum, kn_cross(np,iz), 
     1                         out_ind, out_dim)
c
            points(np, iz) = out_dim
            allocate(prob_in_temp(np)%prob_data(out_dim))
            allocate(indice_in_temp(np)%indice_data(out_dim))

            !read the SRF in a particular column
            call read_HDU3_srf(unit, rownum, out_dim, 
     1           prob_in_temp(np)%prob_data)
c
            do i = 1, out_dim
               indice_in_temp(np)%indice_data(i) = out_ind + (i-1)
            enddo
         enddo
         srf(:,iz)=prob_in_temp
         indices(:,iz)=indice_in_temp
      enddo
...
!-----------------------------------------------------------------------
      ...
      logical :: anynul
      integer :: status,readwrite,blocksize,unit,chdu,hdutype,
     &  colnum,rownum,felem
...
! Read in the function 
      ...
      rownum = 1
      colnum = 1
      call ftgcvd(unit, colnum, rownum, felem, temp_dim, 0.0,
     &     temp, anynul, status)
      colnum = 2
      call ftgcvd(unit, colnum, rownum, felem, en_dim, 0.0,
     &     en, anynul, status)
...
! Read in the function 
      ...
      nelem  = 1
      colnum = 1
      call ftgcvd(unit, colnum, rownum, felem, nelem, 0.0,
     &     kncross, anynul, status)
      if (status .gt. 0) call printerror(status)

      colnum = 2
      call ftgcvj(unit, colnum, rownum, felem, nelem, 0.0,
     &     out_ind, anynul, status)
      if (status .gt. 0) call printerror(status)

      colnum = 3
      call ftgcvj(unit, colnum, rownum, felem, nelem, 0.0,
     &     out_dim, anynul, status)
! Check for any error, and if so print out error messages.
      if (status .gt. 0) call printerror(status)
...
! Read in the function 
      felem  = 1
      colnum = 4
      call ftgcvd(unit, colnum, rownum, felem, out_dim, 0.0,
     &     en, anynul, status)
