      subroutine read_fits(filename, temp_dim, en_dim,temp,en, points, kn_cross, indices, srf)
      ...
      do iz = 1, temp_dim
      	 call read_temp(filename, rownum, iz, unit, en_dim, points(:,iz), kn_cross(:,iz), indices(:,iz), srf(:,iz))
      enddo
...
subroutine read_temp(filename, rownum, iz, unit, en_dim, points, kn_cross, indice_in_temp, prob_in_temp)

      USE structure
      
      implicit none
      
      integer i, iz, np, out_dim, out_ind, rownum, en_dim, unit, points(en_dim)
...
!            print *,prob_in_temp(np)%prob_data(1),'outdim=',out_dim,'np=',np,'iz=',iz
